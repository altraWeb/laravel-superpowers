...
> The same commands work as in-session slash commands too (`/plugin uninstall ...`, `/plugin marketplace remove ...` etc.) if you're already inside Claude Code.
...
After the four steps above, open Claude Code and run:
...
- **Marketplace `altraweb-laravel` already exists after the remove step.** The marketplace name `altraweb-laravel` is reused intentionally by the new host repo. After `claude plugin marketplace remove altraweb-laravel` + `claude plugin marketplace add altraWeb/laravel-marketplace`, the marketplace re-registers under the same name but pointing at the new repo.
- **Old `/laravel-superpowers:status` still works.** This means the V2 plugin is still installed in parallel. Run `claude plugin list` to confirm and `claude plugin uninstall laravel-superpowers` to remove.
